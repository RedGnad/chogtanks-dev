...
public class NetworkBufferedInterpolator : NetworkBehaviour
{
    ...
    void Update()
    {
        if (!Object && stateBuffer.Count >= 2)
        {
            double interpTime = Runner.SimulationTime - interpolationBackTime;

            stateBuffer.RemoveAll(s => s.timestamp < Runner.SimulationTime - bufferTimeLimit);
